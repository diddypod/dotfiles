...
class PlayerManager:
    ...
    def getCurrentPlayer(self):
        playing_players = [
            ...
            if
                self.players[player_owner].status == 'playing'
        ]
        for p in playing_players:
            if "mps-youtube" in self.players[p].bus_name:
                return self.players[p]
        if playing_players:
            return self.players[playing_players[0]]
    ...
    def printStatus(self):
        if self.status in [ 'playing', 'paused' ]:
            ...
            if len(text) == 0 or text == "":
                text = re.sub(FORMAT_REGEX, lambda match: self._statusReplace(match, metadata), '{filename}')
